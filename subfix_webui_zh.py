...
def b_load_list():
    ...
    with open(g_load_file, 'r', encoding="utf-8") as source:
        ...
        for _ in data_list:
            data = _.split('|')
            if (len(data) >= 4):
                wav_path, speaker_name, language= data[:3]
                text = '|'.join(data[3:])
                g_data_json.append(
                        {
                            'wav_path':wav_path,
...
if __name__ == "__main__":
    ...
    parser.add_argument("-p", "--server_port", default=6007,type=int,help="server_port")
    ...
    demo.launch(share=False,server_port=args.server_port)
